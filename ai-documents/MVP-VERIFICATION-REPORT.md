...
### 3.1 — Pagination at scale

Seeded 55 dummy `audit_logs` rows (marked `action='phase3_pagination_test'`), then hit `/admin/activity` (PAGE_SIZE=50):
- Page 1: 50 rows returned, 1.31s (cold), `<details>`-count proof.
- Page 2: 11 rows returned (50 + 11 of 69 total — the rest are real audit rows without metadata block), 0.84s.

Dummy rows deleted after the test (audit_logs back to 14). ✅ Pagination works correctly at scale on a real table.

### 3.2 — Filter / search URL persistence

Hit each filterable admin list with a query-string filter, verified (a) the response only contained matching rows and (b) the rendered `<select>` had the right `<option selected>`:

| URL | Filter persisted? |
|---|---|
| `/admin/rfqs?status=open` | ✅ |
| `/admin/rfqs?status=completed` | ✅ |
| `/admin/suppliers?status=approved` | ✅ |
| `/admin/users?status=supplier` | ✅ |
| `/admin/disputes?tab=resolved` | ✅ |
| `/admin/chats?filter=panic` | ✅ |
| `/admin/chats?filter=archived` | ✅ |
| `/admin/escrow/transactions?status=released` | ✅ |

**8/8** filter URLs persist correctly across SSR + the rendered `<select>` reflects the active option.

### 3.3 — List render times (warm)

Bench script: 2 warmup + 3 timed fetches per page, average reported. Dev-server timings (production will be much faster with build-time prerender + edge cache).

| Page | Persona | Avg | Verdict |
|---|---|---|---|
| `/admin` | admin | 798 ms | ⚠️ dev overhead |
| `/admin/users` | admin | 820 ms | ⚠️ |
| `/admin/suppliers` | admin | 757 ms | ⚠️ |
| `/admin/rfqs` | admin | 800 ms | ⚠️ |
| `/admin/chats` | admin | 800 ms | ⚠️ |
| `/admin/escrow/transactions` | admin | 801 ms | ⚠️ |
| `/admin/activity` | admin | 797 ms | ⚠️ |
| `/ar/dashboard/rfqs` | client | 930 ms | ⚠️ |
| `/ar/supplier/rfqs` | supplier | 990 ms | ⚠️ |
| `/ar/supplier/proposals` | supplier | 1074 ms | 🐢 |
| `/ar/supplier/projects` | supplier | 1215 ms | 🐢 |
| `/ar/supplier/earnings` | supplier | **1279 → 1065 ms** | optimized this section |
| `/ar` (home, public) | — | 50 ms | ✅ |
| `/ar/discover` | — | 182 ms | ✅ |
| `/ar/blog` | — | 36 ms | ✅ |

**Optimization landed**: `/ar/supplier/earnings` was running 3 sequential `escrow_transactions` queries (summary + count + rows) — refactored to `Promise.all` after the agreementIds lookup. Saved ~200ms per render. The other supplier pages cross 1s for the same reason (admin-client + RLS workaround + multiple sequential queries); not optimized this round because the user-facing wait is dominated by Next.js dev-mode compilation rather than DB latency in production. Documented as cross-cutting follow-up for prod profiling.

### 3.4 — Bulk admin actions

Audit of `app/actions/admin.ts`: only single-resource actions exist (`approveSupplierAction`, `rejectSupplierAction`, `cancelRfqAction`, `archiveChatAction`, `overrideRfqStatusAction`). No bulk-select UI in any admin list page (no checkboxes, no select-all). Bulk operations are **not in the MVP doc spec**. At current data volumes (6 suppliers, 3 RFQs, 4 chats, 1 dispute), per-row actions are fine. Documented as Phase 2+ once item counts reach 20+ per queue.

### 3.5 — Lighthouse smoke

Ran headless Lighthouse on the 3 representative pages.

Before:

| Page | Perf | A11y | BP | SEO | Notes |
|---|---:|---:|---:|---:|---|
| `/ar` (home) | 72 | **91** | 96 | 100 | color-contrast + link-name |
| `/ar/dashboard` | 71 | 100 | 96 | 100 | clean |
| `/admin` | 95 | **96** | 96 | **63** | color-contrast + indexable |

**Three real a11y/SEO issues found and fixed**:

1. **`color-contrast`** (home + admin): `--color-stone-600 #7A766F` failed 4.25:1 on cream backgrounds (need 4.5). `--color-success #16A34A` failed 3.29:1 on white. Both bumped darker in [globals.css](app-exhibition-mvp/app/globals.css):
   - `--color-stone-600: #5F5B55` (4.93:1 on cream) ✅
   - `--color-success: #15803D` (4.52:1 on white) ✅
2. **`color-contrast`** (home, `<p class="opacity-90">` on dune-gold + `<span class="opacity-80">` in testimonial figcaption): removed redundant opacity classes. Full-strength text on those backgrounds meets contrast naturally.
3. **`link-name`** (home, `<a href="/">` logo with `aria-hidden` icon span and only mobile-hidden text): added `aria-label={tHome('title')}` to [components/marketing/site-header.tsx](app-exhibition-mvp/components/marketing/site-header.tsx).
4. **SEO `is-crawlable`** (admin): admin shouldn't appear in search results. Added `export const metadata: Metadata = { robots: { index: false, follow: false, noarchive: true, nosnippet: true } }` to [app/admin/layout.tsx](app-exhibition-mvp/app/admin/layout.tsx). Lighthouse penalizes the SEO score for `noindex`, but **that is the correct behavior** for admin — the score drop here is a desired tradeoff and documented.

After:

| Page | Perf | A11y | BP | SEO | Notes |
|---|---:|---:|---:|---:|---|
| `/ar` (home) | 68 | **100** ✅ | 96 | 100 | clean |
| `/ar/dashboard` | 74 | **100** ✅ | 96 | 100 | clean |
| `/admin` | 90 | **100** ✅ | 96 | 63 (intentional `noindex`) | clean a11y |

Performance scores are dev-server numbers (LCP dominated by Next.js dev compile + Turbopack chunk shipping). Production build is out of scope for this verification round.

### 3.6 — Sentry / observability

The app uses a swappable structured-logger pattern:

- [lib/utils/logger.ts](app-exhibition-mvp/lib/utils/logger.ts) — `log.{debug,info,warn,error}` API used app-wide. Default reporter is `consoleReporter` which emits `[app] { level, event, error_message, error_stack, ...context }` per call.
- [instrumentation.ts](app-exhibition-mvp/instrumentation.ts) — Next.js boot hook. When `SENTRY_DSN` is set AND `@sentry/nextjs` is installed, swaps `consoleReporter` for a Sentry reporter that routes `error → captureException`, `warn → captureMessage(level: warning)`. Currently the DSN isn't set, so the in-process default (`consoleReporter`) is what's running.

**Forced-error test** (via `npx tsx scripts/log-test.ts` using the actual `lib/utils/logger.ts`):

```
[app] {
  level: 'error',
  event: 'phase3.regression_test',
  error_message: 'forced error: phase 3 observability check',
  error_stack: 'Error: forced error: phase 3 observability check\n    at <anonymous>...',
  source: 'phase3-test',
  actor: 'admin'
}
[app] { level: 'warn', event: 'phase3.warn_check', reason: '...' }
[app] { level: 'info', event: 'phase3.info_check', ok: true }
```

✅ Pipeline emits at all 3 levels with full stack trace + context attached. Same path is used in production by `safeAfter` (background AI scoring), `audit/record.ts` (audit log failures), `lib/ai/*` (AI gateway errors). Earlier in this verification we already saw `[app] level:error event:ai.score_proposal.failed` fire from a real proposal submission (AI gateway unauthenticated).

#### Phase 3 verdict

✅ All 6 hardening checks complete. **Fixes shipped**: 3 site-wide a11y wins (color-contrast on 2 tokens, header link aria-label, opacity removal on home), `noindex` on admin, earnings-page query parallelization (~200ms saved per render).
