...
<head><meta charset="utf-8"><meta name="x-apple-disable-message-reformatting"><title>${preview}</title></head>
...
export function rfqMatchEmail(v: RfqMatchVars): { subject: string; html: string } {
  ...
  const body = `
    ...
    <p style="font-size:14px;line-height:1.6;color:${COLORS.charcoal};margin:0 0 12px 0;">مرحباً ${v.supplierName}،</p>
    ...
    <div style="background:${COLORS.cream};padding:16px;border-radius:8px;margin-bottom:16px;font-size:13px;">
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span style="color:${COLORS.stone600};">رقم الطلب</span><span style="color:${COLORS.charcoal};font-weight:600;">${v.rfqNumber}</span></div>
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span style="color:${COLORS.stone600};">نوع الخدمة</span><span style="color:${COLORS.charcoal};font-weight:600;">${v.serviceTypeAr}</span></div>
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span style="color:${COLORS.stone600};">المدينة</span><span style="color:${COLORS.charcoal};font-weight:600;">${v.city}</span></div>
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span style="color:${COLORS.stone600};">الميزانية</span><span style="color:${COLORS.charcoal};font-weight:600;">${v.budgetRange ?? 'لم تُحدد'}</span></div>
      <div style="display:flex;justify-content:space-between;padding:4px 0;"><span style="color:${COLORS.stone600};">آخر موعد</span><span style="color:${COLORS.charcoal};font-weight:600;">${v.deadline}</span></div>
    </div>
    <div style="text-align:center;padding:16px 0;">
      <a href="${v.rfqUrl}" style="display:inline-block;padding:10px 20px;background:${COLORS.midnight};color:${COLORS.cream};text-decoration:none;border-radius:8px;font-size:14px;font-weight:600;">افتح الطلب وقدّم عرضك ←</a>
    </div>
  `;
