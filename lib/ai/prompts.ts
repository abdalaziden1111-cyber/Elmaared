...
/**
 * V1.2 — extended system prompt with Saudi commercial-law context injected.
 * The model uses the templates as a baseline to flag deviations as
 * `risky_clauses`. Keep the templates separable so prompt diffing in
 * `git` stays readable.
 */
export function buildAnalyzeAgreementSystem(legalContext: string): string {
  return [
    ANALYZE_AGREEMENT_SYSTEM,
    '',
    'بالإضافة، استخرج البنود الخطرة (risky_clauses) — أي بند في فهم العميل أو المورد ينحرف عن المعتاد في السوق السعودي بناءً على المرجع التالي.',
    'لكل بند خطر: clause (نص قصير يصف البند)، deviation (شرح موجز لكيفية الانحراف)، severity (high|medium|low).',
    'إذا لم تجد بنوداً خطرة، أعد risky_clauses كمصفوفة فارغة.',
    '',
    legalContext,
  ].join('\n');
}
